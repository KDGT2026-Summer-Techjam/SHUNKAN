...
- 温かいオフホワイト、深いネイビー、ロゴ（旬間Logo）のアクセントブルーを共通色とする。アクセントカラーはファビコン・ロゴから抽出した`#0E3B5D`（ネイビー）と`#39638A`（ブルー）を使う。
- 認証画面（ログイン・新規登録・ホーム）は、文字組みではなく `static/core/images/shunkan-logo.png` のロゴ画像を表示する。
- 下部ナビは元デザインに合わせて画面下端に密着した全幅バーとし、中央の撮影ボタンだけ白い円形で上に飛び出させる。
